...
class WalkUser(models.Model, DataProxy):
    ...
    register_type = fields.Selection(RegisterType, string='注册来源', default='app')
    ...
    _sql_constraints = [
     ('wxapp_user_union_id_unique', 'UNIQUE (union_id, create_uid)', 'wechat user union_id with create_uid is existed！'),
     ('wxapp_user_open_id_unique', 'UNIQUE (open_id, create_uid)', 'wechat user open_id with create_uid is existed！')]
    ...
    def _compute_avatar(self):
        for each_record in self:
            if each_record.avatar_url:
                each_record.avatar = '<img src="{avatar_url}" style="max-width:100px;">'.format(
                    avatar_url=each_record.avatar_url
                )
            else:
